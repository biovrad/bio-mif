<?php get_header(); ?>
	...
	<?php get_sidebar("left"); ?>
				<div id="center">
	<?php if (have_posts()) : ?>
		<h2 class="center"><?php _e('Результаты поиска', 'kubrick'); ?></h2>
		<?php while (have_posts()) : the_post(); ?>
			...
			<div class="post" id="post-<?php the_ID(); ?>">
				...
				<p class="postmetadata"><?php the_tags(__('Теги:', 'kubrick') . ' ', ', ', '<br />'); ?> <?php printf(__('Опубликовано в  %s', 'kubrick'), get_the_category_list(', ')); ?> | <?php edit_post_link(__('Изменить', 'kubrick'), '', ' | '); ?>  <?php comments_popup_link(__('Нет коментариев &#187;', 'kubrick'), __('1 Коментарий &#187;', 'kubrick'), __('% Коментария(ев) &#187;', 'kubrick'), '', __('Коментарии недоступны', 'kubrick') ); ?></p>
			</div>
